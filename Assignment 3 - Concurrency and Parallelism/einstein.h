...
const unsigned int NUM_THREAD = 5;
const unsigned int NUM_IDX_PER_JOB = 5;
...
class job {
    ...
    public:
        // Constructors
        job(): max_pos(0){ }
        job(std::vector<size_t>& indexes, unsigned int max_pos): max_pos(max_pos), indexes(indexes){ }
    
        // "indexes" is the vector containing the initial position of the indexes that a job can manage (and compute)
        std::vector<size_t> indexes;
    
        // Methods for "n"
        void increment_max_pos(){ ++max_pos; }
        
        void set_max_pos(unsigned int x){ max_pos = x; }
        
        unsigned int get_max_pos(){ return max_pos; }
    
    private:
        // "ind" is the number of position that a job can manage (and compute)
...
class worker_thread {
    public:
        // Function my_thread (executed as thread)
        static void my_thread(std::vector<job<T1>>& job_list,
            ...
            worker_thread w;
            while (w.get_my_job(my_job, job_list, jobs_mutex) == true){
                std::cout << "Thread " << std::this_thread::get_id() << " is running\n";
                
                unsigned int i = 0;
                
                while (i < my_job.get_max_pos()) {
                    T1& value = dest.eval(my_job.indexes);
                    eval_ris = source.eval(my_job.indexes);
                    
                    // START Critical section: use a lock guard with mutex
                    std::unique_lock<std::mutex> my_lock_mutex(*(dest_mutex[&value-dest.start_ptr]));
                    value += eval_ris;
                    my_lock_mutex.unlock();
                    // END Critical section
                    
                    dest.next(my_job.indexes);
                    ++i;
                }
            }
        }
    ...
    private:
        ...
        bool get_my_job(job<T1>& j, std::vector<job<T1>>& job_list, std::mutex& jobs_mutex){
            
            if (!job_list.empty()){
                std::unique_lock<std::mutex> my_lock_jobs(jobs_mutex);
                
                // START Critical section: use a lock guard with mutex
                j = job_list.back();  // get the last element
                job_list.pop_back();    // remove the last element
                my_lock_jobs.unlock();
                // END Critical section
...
class scheduler{
    ...
    public:
        // Constructor
        scheduler(std::vector<job<T1>>& job_list,
                  einstein_expression<T1,dynamic>& dest,
                  einstein_expression<T2,dynamic,C2>& source,
                  std::map<unsigned int, std::unique_ptr<std::mutex>>& dest_mutex) : job_list(job_list), dest(dest), source(source), dest_mutex(dest_mutex)  { }

        // Function that creates, runs and waits for all threads
        void run(){
            // Create threads and run them (assign a worker_thread for each thread)
            for (int i=0; i!=thread_list.size(); ++i){
                thread_list[i] = std::thread(Tensor::worker_thread<T1, C1, T2, C2>::my_thread,
                                             std::ref(job_list),
                                             std::ref(dest),
                                             std::ref(source),
                                             std::ref(jobs_mutex),
    ...
    private:
        ...
        // Scheduler maintains the source and the destination tensor to be performed
        einstein_expression<T1,dynamic>& dest;
        einstein_expression<T2,dynamic,C2> source;
        
        std::mutex jobs_mutex;   // mutex to access to job list
        std::map<unsigned int, std::unique_ptr<std::mutex>>& dest_mutex;  // mutex to access to destination tensor
};
...
public:
    ...
    einstein_expression<T,dynamic>& operator =(einstein_expression<T2,dynamic,TYPE2>&& x) {
        ...
        while (!end(indexes)) {
            T& value = eval(indexes);
            value = 0;
            
            dest_mutex[&value-start_ptr] = std::make_unique<std::mutex>();  // since std::mutex is not movable and not copyable
            next(indexes);
        }
        ...
        // Setup after aligning
        setup(indexes);
        x.setup(indexes);
        
        job<T> j;
        
        while (end(indexes) == false) {
            if (j.get_max_pos() == 0){
                j.indexes = indexes;
            }
            j.increment_max_pos();
            if (j.get_max_pos() == NUM_IDX_PER_JOB){
        ...
        scheduler<T, einstein_proxy, T2, TYPE2> scheduler(job_list, *this, x, dest_mutex);
        scheduler.run();
    ...
    template<typename T1, class TYPE1, typename T2, class TYPE2> friend class scheduler;
    template<typename T1, class TYPE1, typename T2, class TYPE2> friend class worker_thread;
...
protected:
    ...
    void setup(std::vector<size_t>& idxs) {
        widths.clear();
        strides.clear();
        idxs.clear();
    ...
    void reset(std::vector<size_t>& idxs) {
        for (auto i=idxs.begin(); i!=idxs.end(); ++i) *i=0;
        current_ptr=start_ptr;
    }

    bool end(std::vector<size_t>& idxs) {
        return idxs[0]==widths[0];
    }

    T& eval(std::vector<size_t> idxs) {
        
        assert(idxs.size()==index_map.size());
        T* current_ptr = start_ptr;
        
        for (size_t i=0; i<idxs.size(); ++i) {
            current_ptr += idxs[i] * strides[i];
        }
        
        return *current_ptr;
    }

    void next(std::vector<size_t>& idxs){
...
protected:
    ...
    bool end(std::vector<size_t>& idxs) { return exp1.end(idxs); }
    ...
    T eval(std::vector<size_t> idxs) {
        return exp1.eval(idxs) * exp2.eval(idxs);
